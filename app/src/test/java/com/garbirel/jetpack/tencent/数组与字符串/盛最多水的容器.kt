...
class 盛最多水的容器 {
    ...
    fun maxArea2 (height: IntArray) : Int {
        ...
        for(index in height.indices) {
            var right = height.size - 1
            while(index < right) {
                max = Math.max((right - index) * Math.min(height[index],height[right]),max)
                right--
            }
        }
        return max
